...
function render() {
  ...
  // $("#browse-info").append(title)
  //                  .append("<hr/>")
  //                  .append(overview);
    //  $("#add-to-watchlist").unbind("click")
    //                         .click(function(){
    ...
    $("#add-to-watchlist")
         ...
         var posters = model.browseItems.map(function(movie){
            //  var posters_img = api.posterUrl(movie);
            //  return $("<div/>").addClass("item").append($("<img/>")).attr("src",api.posterUrl(movie));
            var poster = $("<img></img>")
              .attr("src", api.posterUrl(movie))
              .attr("class", "img-responsive");
